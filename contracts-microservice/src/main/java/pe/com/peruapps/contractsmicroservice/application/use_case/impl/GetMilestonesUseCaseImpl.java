...
import pe.com.peruapps.contractsmicroservice.application.use_case.GetMilestonesUseCase;
...
public class GetMilestonesUseCaseImpl implements GetMilestonesUseCase {
  ...
  public List<MilestoneResponse> execute(Contract.ContractId contractId) {
    contractRepository.findById(contractId)
        .orElseThrow(() -> new ContractNotFound(contractId,
            "Contract not found with id: " + contractId.value()));

    return performanceRepository.findMilestonesByContractId(contractId).stream()
        .map(mapper::toMilestoneResponse)
        .toList();
  }
}
